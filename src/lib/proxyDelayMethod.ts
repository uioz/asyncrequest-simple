 ...
 * 本类提供了方法节流和方法防抖的支持
 */
export class ProxyDelayMethod<T> {

    private obj: T;
    private baseTime: number = 200;
    private throttle: {
        [propName: string]: {
            method: any;
            delayTime: number;
     ...
     */
    constructor(object: T, baseTime?: number) {
     ...
     */
    setDebounceMethods(propName: string, delayTime?: number) {
     ...
     * @param delayTime 超时时间,多少时间后执行
     */
    setDelayExecuteOnceMethod(propName: string, delayTime?: number) {
     ...
     */
    setThrottleMethods(propName: string, delayTime?: number): void {
        ...
        this.obj[propName] = (...args) => {
            ...
            // 3. 循环调用函数
            function call(this: ProxyDelayMethod<T>, that, hander, obj) {
                "use strict";
