 ...
 */
export class DelayTriggerQueue {
     ...
     */
    hasTaskQueue(queueName: string): boolean {
     ...
     */
    setTask(taskName: string, delayTime = 10000,method,bind = undefined) {
     ...
     * @param args 参数
     */
    push(queueName: string,...args): boolean {
    ...
    private execute(taskName) {
        "use strict";
        let taskBody = this.taskInf[taskName],
            that = this;
